(function ($) {
    ...
    function galleryManager(el, options) {
        ...
        var $progressOverlay = $('.progress-modal', $gallery);
        ...
        function createEditorElement(id, src, name, description) {
            var html = '<div class="photo-editor">' +
                '<div class="preview col-md-4"><img src="' + htmlEscape(src) + '" alt=""/></div>' +
                '<div class="col-md-7">' +
                '<div class="col-md-12">' +
                (opts.hasName
                    ? '<label for="photo_name_' + id + '">' + opts.nameLabel + ':</label>' +
                    '<input type="text" name="photo[' + id + '][name]" class="form-control" value="' + htmlEscape(name) + '" id="photo_name_' + id + '"/>'
                    : '') +
                '</div>' +
                '<div class="col-md-12">' +
                (opts.hasDesc
                    ? '<label for="photo_description_' + id + '">' + opts.descriptionLabel + ':</label>' +
                    '<textarea name="photo[' + id + '][description]" style="height: 60px;" class="form-control" id="photo_description_' + id + '">' + htmlEscape(description) + '</textarea>'
                    : '') +
                '</div>' +
                '</div>' +
                '</div>';
        ...
        if (opts.hasName || opts.hasDesc)photoTemplate += '<span class="editPhoto btn btn-primary btn-sm"><i class="fa fa-pencil"></i></span> ';
        photoTemplate += '<span class="deletePhoto btn btn-danger btn-sm"><i class="fa fa-trash icon-white"></i></span>' +
            '</div><div class="form-group"><div class="checkbox"><label><input type="checkbox" class="photo-select"/></label></div></div></div>';
        ...
        if (window.FormData !== undefined) { // if XHR2 available
            ...
            function multiUpload(files) {
                ...
                $progressOverlay.modal('show');
                $uploadProgress.css('width', '5%');
                $uploadProgress.attr('aria-valuenow', '5');
                $uploadProgress.text('5%');
                ...
                for (var i = 0; i < filesCount; i++) {
                    ...
                    xhr.onload = function () {
                        ...
                        var proc = (5 + 95 * uploadedCount / filesCount);
                        $uploadProgress.css('width', '' + proc + '%');
                        $uploadProgress.attr('aria-valuenow', proc);
                        $uploadProgress.text(parseInt(proc) + '%');

                        if (uploadedCount === filesCount) {
                            $uploadProgress.css('width', '100%');
                            $uploadProgress.attr('aria-valuenow', '100');
                            $uploadProgress.text('100%');
                            $progressOverlay.modal('hide');
